...
/**
 * @brief Interface abstraction for socket operations to enable dependency injection and testing.
 * 
 * This interface abstracts all socket operations used by the TCP cluster manager,
 * allowing for mock implementations in unit tests and fake implementations in integration tests.
 */
...
public:
    ...
    // Socket options
    virtual int setSockOpt(vgre_socket_t fd, int level, int optname, const void* optval, socklen_t optlen) = 0;
    virtual int getSockOpt(vgre_socket_t fd, int level, int optname, void* optval, socklen_t* optlen) = 0;
};

/**
 * @brief Interface abstraction for memory management operations to enable dependency injection and testing.
 * 
 * This interface abstracts memory management operations used by the TCP cluster manager,
 * allowing for mock implementations that can simulate different memory scenarios in tests.
 */
...
public:
    ...
    virtual void getDirtyPages(void* ptr, std::vector<std::pair<size_t, size_t>>& dirtyRanges) = 0;
...
/**
 * @brief Interface abstraction for secure channel creation to enable dependency injection and testing.
 * 
 * This interface abstracts secure channel creation used by the TCP cluster manager,
 * allowing for mock implementations that can simulate different security scenarios in tests.
 */
 ...
 * 
 * This is the production implementation that delegates to actual socket system calls.
 * Used by the singleton instance() method for normal operation.
 */
 ...
 * 
 * This is the production implementation that delegates to the actual memory manager
 * from the RuntimeEngine. Used by the singleton instance() method for normal operation.
 */
...
/**
 * @brief Real implementation of ISecureChannelFactory creating actual SecureChannel instances.
 * 
 * This is the production implementation that creates real SecureChannel instances.
 * Used by the singleton instance() method for normal operation.
 */
