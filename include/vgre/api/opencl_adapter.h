...
public:
    ...
    // ── Command queue ──────────────────────────────────────────────────────
    cl_command_queue createCommandQueue(cl_context ctx,
                                        cl_device_id device,
                                        cl_int* errcode);
    cl_int releaseCommandQueue(cl_command_queue queue);

    // ── Memory ─────────────────────────────────────────────────────────────
    cl_mem createBuffer(cl_context ctx, cl_int flags, size_t size,
    ...
    cl_int enqueueWriteBuffer(cl_command_queue queue, cl_mem buffer,
                              ...
                              const void* ptr);
    cl_int enqueueReadBuffer(cl_command_queue queue, cl_mem buffer,
                             size_t offset, size_t size,
                             void* ptr);

    // ── Program ────────────────────────────────────────────────────────────
    cl_program createProgramWithSource(cl_context ctx,
    ...
    // ── Kernel ─────────────────────────────────────────────────────────────
    cl_kernel_handle createKernel(cl_program program,
                                  const std::string& name,
                                  cl_int* errcode);
    cl_int setKernelArg(cl_kernel_handle kernel, cl_uint argIndex,
                        size_t argSize, const void* argValue);
    cl_int enqueueNDRangeKernel(cl_command_queue queue,
