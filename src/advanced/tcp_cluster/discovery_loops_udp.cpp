...
void DiscoveryManager::udpDiscoveryLoop() {
  if (!parent_->data_processor_thread_.joinable()) {
      parent_->data_processor_thread_ = std::thread(&TCPClusterManager::processClientStagingBuffer, parent_);
  }

  while (parent_ && parent_->enabled_) {
    vgre::common::vgre_socket_t udp_fd = socket(AF_INET, SOCK_DGRAM, 0);
    if (udp_fd == vgre::common::VGRE_INVALID_SOCKET) { std::this_thread::sleep_for(std::chrono::seconds(2)); continue; }
    ...
    if (parent_->has_master_fd_.load()) {
        while (parent_ && parent_->enabled_) {
            { std::lock_guard<std::mutex> lk(parent_->client_mutex_); if (parent_->client_fd_ == vgre::common::VGRE_INVALID_SOCKET) break; }
