...
void TCPClusterManager::handleClientDataEvents(std::vector<vgre_pollfd>& fds) {
    ...
    for (size_t i = 1; i < fds.size(); ++i) {
        ...
        for (auto &client : clients_) {
            if (!client || client->socket_fd != fds[i].fd) continue;
            ...
            if (fds[i].revents & POLLIN) {
                ...
                int n = recv_packet(client->socket_fd, client->rx_buffer, client->secure_channel.get());
                if (n < 0) {
                    client->active = false; vgre::common::vgre_close_socket(client->socket_fd);
                    client->socket_fd = vgre::common::VGRE_INVALID_SOCKET;
                    syncToIPC();
                    std::lock_guard<std::mutex> bk(proactive_backoff_mutex_);
                    proactive_backoff_until_[client->ip_address] = std::chrono::steady_clock::now() + std::chrono::seconds(8);
                }
            } else {
                client->active = false; vgre::common::vgre_close_socket(client->socket_fd);
                client->socket_fd = vgre::common::VGRE_INVALID_SOCKET;
