    ...
    while (enabled_ && !client_rx_buffer_.empty()) {
        if (receive_state_ == ReceiveState::IDLE) {
            ...
            if (type == PacketType::ARG_SCALAR || type == PacketType::ARG_POINTER) {
                ...
                if (client_rx_buffer_.size() < sizeof(VSBPHeader) + sizeof(DataHeaderRDMAPacket)) break;
                DataHeaderRDMAPacket rdmaPkt;
                std::memcpy(&rdmaPkt, client_rx_buffer_.data() + sizeof(VSBPHeader), sizeof(DataHeaderRDMAPacket));
                client_rx_buffer_.erase(client_rx_buffer_.begin(),
                    client_rx_buffer_.begin() + sizeof(VSBPHeader) + sizeof(DataHeaderRDMAPacket));
                ...
                if (client_rdma_connected_ && client_rdma_conn_ &&
                    ...
                    void* local_ptr = mm.getPointer(handle);
                    if (local_ptr) {
                        std::memcpy(local_ptr, client_rdma_conn_->bounceBuf(), rdmaPkt.size);
                    ...
                    VGRE_LOG_WARN("TCPCluster",
                        ...
                        std::to_string(rdmaPkt.size) + " exceeds bounce capacity — data lost");
                }
