/**
 * Production implementations of the TCP cluster dependency-injection interfaces.
 *
 * RealSocketFactory    — delegates to POSIX / Winsock system calls with validation
 * RealMemoryManager    — delegates to RuntimeEngine::getMemoryManager() with error handling
 * RealSecureChannelFactory — creates real SecureChannel instances with configuration validation
 */
...
vgre_socket_t RealSocketFactory::createSocket(int domain, int type, int protocol) {
    // Validate socket parameters
    if (domain != AF_INET && domain != AF_INET6 && domain != AF_UNIX) {
        VGRE_LOG_ERROR("RealSocketFactory", "Invalid socket domain: " + std::to_string(domain));
        errno = EAFNOSUPPORT;
    ...
    if (fd == VGRE_INVALID_SOCKET) {
        VGRE_LOG_ERROR("RealSocketFactory", "Failed to create socket: " + std::string(strerror(errno)));
    }
    return fd;
}

int RealSocketFactory::bind(vgre_socket_t fd, const sockaddr* addr, socklen_t len) {
    ...
    if (result != 0) {
        VGRE_LOG_ERROR("RealSocketFactory", "Bind failed: " + std::string(strerror(errno)));
    }
    return result;
}
...
int RealSocketFactory::listen(vgre_socket_t fd, int backlog) {
    ...
    if (result != 0) {
        VGRE_LOG_ERROR("RealSocketFactory", "Listen failed: " + std::string(strerror(errno)));
    }
    return result;
}

vgre_socket_t RealSocketFactory::accept(vgre_socket_t fd, sockaddr* addr, socklen_t* len) {
    if (fd == VGRE_INVALID_SOCKET) {
    ...
    if (client_fd == VGRE_INVALID_SOCKET) {
        ...
        if (errno != EAGAIN && errno != EWOULDBLOCK) {
            VGRE_LOG_ERROR("RealSocketFactory", "Accept failed: " + std::string(strerror(errno)));
        }
    }
    return client_fd;
}

int RealSocketFactory::connect(vgre_socket_t fd, const sockaddr* addr, socklen_t len) {
    if (fd == VGRE_INVALID_SOCKET || !addr || len == 0) {
    ...
    if (result != 0) {
        ...
        if (errno != EINPROGRESS) {
            VGRE_LOG_ERROR("RealSocketFactory", "Connect failed: " + std::string(strerror(errno)));
        }
    }
    return result;
}

int RealSocketFactory::send(vgre_socket_t fd, const void* buf, size_t len, int flags) {
    ...
    if (len > static_cast<size_t>(std::numeric_limits<int>::max())) {
        VGRE_LOG_ERROR("RealSocketFactory", "Send length too large: " + std::to_string(len));
        errno = EMSGSIZE;
        return -1;
    }
    ...
    if (result < 0) {
        // Don't log EAGAIN/EWOULDBLOCK as errors (normal for non-blocking sockets)
        if (errno != EAGAIN && errno != EWOULDBLOCK) {
            VGRE_LOG_ERROR("RealSocketFactory", "Send failed: " + std::string(strerror(errno)));
        }
    }
    return result;
}

int RealSocketFactory::recv(vgre_socket_t fd, void* buf, size_t len, int flags) {
    if (fd == VGRE_INVALID_SOCKET || !buf) {
    ...
    // Prevent integer overflow on Windows
    if (len > static_cast<size_t>(std::numeric_limits<int>::max())) {
        VGRE_LOG_ERROR("RealSocketFactory", "Recv length too large: " + std::to_string(len));
        errno = EMSGSIZE;
        return -1;
    }
    ...
    if (result < 0) {
        // Don't log EAGAIN/EWOULDBLOCK as errors (normal for non-blocking sockets)
        if (errno != EAGAIN && errno != EWOULDBLOCK) {
            VGRE_LOG_ERROR("RealSocketFactory", "Recv failed: " + std::string(strerror(errno)));
        }
    }
    return result;
}
...
int RealSocketFactory::poll(vgre_pollfd* fds, int nfds, int timeout) {
    ...
    if (nfds < 0) {
        VGRE_LOG_ERROR("RealSocketFactory", "Invalid nfds for poll: " + std::to_string(nfds));
        errno = EINVAL;
        return -1;
    }
    ...
    if (result < 0) {
        VGRE_LOG_ERROR("RealSocketFactory", "Poll failed: " + std::string(strerror(errno)));
    }
    return result;
}
...
int RealSocketFactory::setSockOpt(vgre_socket_t fd, int level, int optname,
    ...
    int result = vgre::common::vgre_setsockopt(fd, level, optname, optval, optlen);
    if (result != 0) {
        VGRE_LOG_ERROR("RealSocketFactory", "SetSockOpt failed: " + std::string(strerror(errno)));
    }
    return result;
}
...
int RealSocketFactory::getSockOpt(vgre_socket_t fd, int level, int optname,
    ...
    int result;
#ifdef _WIN32
    result = ::getsockopt(fd, level, optname, static_cast<char*>(optval), optlen);
#else
    result = ::getsockopt(fd, level, optname, optval, optlen);
#endif
    
    if (result != 0) {
        VGRE_LOG_ERROR("RealSocketFactory", "GetSockOpt failed: " + std::string(strerror(errno)));
    }
    return result;
}
...
size_t RealMemoryManager::getAllocationSize(void* ptr) {
    if (!ptr) {
        VGRE_LOG_ERROR("RealMemoryManager", "Null pointer passed to getAllocationSize");
        return 0;
    }
    
    try {
        return vgre::core::RuntimeEngine::instance().getMemoryManager().getAllocationSize(ptr);
    } catch (const std::exception& e) {
        VGRE_LOG_ERROR("RealMemoryManager", "Exception in getAllocationSize: " + std::string(e.what()));
        return 0;
    }
}
...
void* RealMemoryManager::getPointer(uint64_t handle) {
    if (handle == 0) {
        VGRE_LOG_ERROR("RealMemoryManager", "Invalid handle (0) passed to getPointer");
        return nullptr;
    }
    
    try {
        return vgre::core::RuntimeEngine::instance().getMemoryManager()
                   .getPointer(reinterpret_cast<void*>(handle));
    } catch (const std::exception& e) {
        VGRE_LOG_ERROR("RealMemoryManager", "Exception in getPointer: " + std::string(e.what()));
        return nullptr;
    }
}
...
bool RealMemoryManager::isValidHandle(uint64_t handle) {
    ...
    try {
        ...
        VGRE_LOG_ERROR("RealMemoryManager", "Exception in isValidHandle: " + std::string(e.what()));
        return false;
    }
}
...
uint64_t RealMemoryManager::allocateManagedAt(void* ptr, size_t size) {
    if (!ptr || size == 0) {
        VGRE_LOG_ERROR("RealMemoryManager", "Invalid parameters for allocateManagedAt");
        return 0;
    }
    
    try {
        MemoryHandle outHandle = nullptr;
        VGREResult result = vgre::core::RuntimeEngine::instance().getMemoryManager()
            .allocateManagedAt(ptr, size, outHandle);
        
        if (result != VGREResult::SUCCESS) {
            VGRE_LOG_ERROR("RealMemoryManager", "allocateManagedAt failed with result: " + std::to_string(static_cast<int>(result)));
            return 0;
        }
        
        return reinterpret_cast<uint64_t>(outHandle);
    } catch (const std::exception& e) {
        VGRE_LOG_ERROR("RealMemoryManager", "Exception in allocateManagedAt: " + std::string(e.what()));
        return 0;
    }
}

void RealMemoryManager::getDirtyPages(void* ptr,
    ...
    try {
        vgre::core::RuntimeEngine::instance().getMemoryManager().getDirtyPages(ptr, dirtyRanges);
    } catch (const std::exception& e) {
        VGRE_LOG_ERROR("RealMemoryManager", "Exception in getDirtyPages: " + std::string(e.what()));
        dirtyRanges.clear();
    }
}
...
void RealMemoryManager::clearDirtyPages(void* ptr) {
    if (!ptr) {
        VGRE_LOG_ERROR("RealMemoryManager", "Null pointer passed to clearDirtyPages");
        return;
    }
    
    try {
        vgre::core::RuntimeEngine::instance().getMemoryManager().clearDirtyPages(ptr);
    } catch (const std::exception& e) {
        VGRE_LOG_ERROR("RealMemoryManager", "Exception in clearDirtyPages: " + std::string(e.what()));
    }
}
...
VGREResult RealMemoryManager::mapMemory(void* ptr, size_t size) {
    ...
    try {
        ...
        size_t allocSize = vgre::core::RuntimeEngine::instance().getMemoryManager().getAllocationSize(ptr);
        if (allocSize == 0) {
            VGRE_LOG_ERROR("RealMemoryManager", "mapMemory called on unmanaged pointer");
            return VGREResult::ERR_INVALID_VALUE;
        }
        
        if (size > allocSize) {
            VGRE_LOG_WARN("RealMemoryManager", "mapMemory size (" + std::to_string(size) + 
                            ") exceeds allocation size (" + std::to_string(allocSize) + ")");
        }
        
        return VGREResult::SUCCESS;
    } catch (const std::exception& e) {
        VGRE_LOG_ERROR("RealMemoryManager", "Exception in mapMemory: " + std::string(e.what()));
...
VGREResult RealMemoryManager::unmapMemory(void* ptr) {
    ...
    // Similar to mapMemory, this is a no-op for UVM but we validate for robustness
    try {
        size_t allocSize = vgre::core::RuntimeEngine::instance().getMemoryManager().getAllocationSize(ptr);
        if (allocSize == 0) {
            VGRE_LOG_ERROR("RealMemoryManager", "unmapMemory called on unmanaged pointer");
            return VGREResult::ERR_INVALID_VALUE;
        }
        
        return VGREResult::SUCCESS;
    } catch (const std::exception& e) {
        VGRE_LOG_ERROR("RealMemoryManager", "Exception in unmapMemory: " + std::string(e.what()));
        return VGREResult::ERR_UNKNOWN;
    }
}
...
VGREResult RealSecureChannelFactory::validateConfiguration() {
    ...
    if (!ch) return VGREResult::ERR_NOT_INITIALIZED;
