...
void TCPClusterManager::processServerPackets(std::shared_ptr<ClientConnection> client) {
            ...
            if (client->receive_state == ReceiveState::IDLE) {
              ...
              if (type == PacketType::TELEMETRY) {
                ...
                if (client->rdma_connected && client->rdma_conn &&
                    ...
                    if (local_ptr) {
                        std::memcpy(local_ptr, client->rdma_conn->bounceBuf(), rdmaPkt.size);
                        VGRE_LOG_DEBUG("TCPCluster",
                            "Master: RDMA receive " + std::to_string(rdmaPkt.size) +
                            " bytes from " + client->ip_address +
                            " → handle " + std::to_string(rdmaPkt.target_ptr));
                    } else {
                        VGRE_LOG_ERROR("TCPCluster",
                            "Master: DATA_HEADER_RDMA: no local pointer for handle " +
                            std::to_string(rdmaPkt.target_ptr) + " from " + client->ip_address);
                    }
                } else {
                    VGRE_LOG_WARN("TCPCluster",
                        "Master: DATA_HEADER_RDMA from " + client->ip_address +
                        " but RDMA not active or size " + std::to_string(rdmaPkt.size) +
                ...
                client->rx_buffer.erase(client->rx_buffer.begin(), client->rx_buffer.begin() + totalLen);
                if (client->secure_channel) client->secure_channel->rotateKey(rpkt.nonce);
