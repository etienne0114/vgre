...
VGREResult TCPClusterManager::initialize(bool is_master, const std::string &host, int port) {
  if (enabled_ && is_master_ == is_master && port_ == port) return VGREResult::SUCCESS;
  shutdown();
  ...
  if (is_master_) {
    ...
    int opt = 1; 
    vgre::common::vgre_setsockopt(server_fd_, SOL_SOCKET, SO_REUSEADDR, (const char*)&opt, sizeof(opt));
    ...
    if (bind(server_fd_, (struct sockaddr *)&addr, sizeof(addr)) < 0 || listen(server_fd_, 128) < 0) {
    ...
    VGRE_LOG_INFO("TCPCluster", "Master node listening on port " + std::to_string(port_));
    ...
    VGRE_LOG_INFO("TCPCluster", "Worker node initializing (master=" + host_ + ":" + std::to_string(port_) + ")");
