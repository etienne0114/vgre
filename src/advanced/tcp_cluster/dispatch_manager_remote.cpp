...
VGREResult DispatchManager::launchRemoteKernel(int worker_idx, uint64_t kernel_id, const uint32_t grid_dim[3], const uint32_t block_dim[3], void** args, int num_args, size_t shared_mem) {
  if (!parent_->is_master_) return VGREResult::ERR_INVALID_VALUE;
  
  std::lock_guard<std::recursive_mutex> lock(parent_->clients_mutex_);
  if (worker_idx < 0 || worker_idx >= static_cast<int>(parent_->clients_.size()) || !parent_->clients_[worker_idx] || !parent_->clients_[worker_idx]->active) {
    return VGREResult::ERR_INVALID_VALUE;
  }

  // Stream arguments
  VGREResult argResult = parent_->streamArgumentsToWorker(args, num_args, kernel_id, parent_->clients_[worker_idx]);
  if (argResult != VGREResult::SUCCESS) return argResult;
...
void DispatchManager::handleRemoteCommand(const RemoteCommandPacket& pkt) {
  ...
  for (int i = 0; i < pkt.num_args; ++i) {
    ...
    if (it != parent_->pending_args_.end()) {
        if (it->second.type == (uint8_t)ArgType::STRUCT) local_args[i] = it->second.data.data();
        else local_args[i] = &it->second.value;
    }
  }
  ...
  VGREResult r = vgre::core::RuntimeEngine::instance().launchKernel(pkt.kernel_id, gd, bd, local_args.data(), pkt.shared_mem, 0);

  ResponsePacket resp{pkt.kernel_id, r};
  parent_->send_packet(parent_->client_fd_, PacketType::RESPONSE, &resp, sizeof(ResponsePacket), parent_->client_secure_channel_.get());
...
VGREResult DispatchManager::waitForRemoteResult(uint64_t kernel_id, int timeout_ms) {
    ...
    while (remote_kernel_results_.find(kernel_id) == remote_kernel_results_.end()) {
        if (parent_->remote_results_cv_.wait_until(lock, deadline) == std::cv_status::timeout) return VGREResult::ERR_TIMEOUT;
    }
