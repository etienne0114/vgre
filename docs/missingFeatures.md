...
|---|---|---|
| CUDA Runtime API | ~45% | 95 functions implemented; ~119+ missing including `cudaEventQuery`, `cudaMemcpyToSymbol`, `cudaFuncGetAttributes`, texture/surface objects, array APIs, graph kernel nodes, callbacks, stream capture introspection |
...
1. **Immediate (blocks PyTorch/TF graphs)**: Implement `cudaGraphAddKernelNode`, `cudaMemcpyToSymbol`, `cudaEventQuery`, `cudaFuncGetAttributes`.
