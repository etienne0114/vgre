...
|---|---|---|
| CUDA Runtime API | ~45% | 94 functions implemented; ~120+ missing including `cudaStreamWaitEvent`, `cudaEventQuery`, `cudaMemcpyToSymbol`, `cudaFuncGetAttributes`, texture/surface objects, array APIs, graph kernel nodes, callbacks, stream capture introspection |
...
|---|---|---|---|
| 1.1.1 | `cudaStreamWaitEvent` | **CRITICAL** — every async framework uses this for cross-stream ordering. Only `cuStreamWaitEvent` exists in driver shim; CUDART shim does **not** expose it. | `grep -rn "^cudaError_t cudaStreamWaitEvent" src/api/cudart_shim*` → no match |
| 1.1.2 | `cudaEventQuery` | High — non-blocking event status checks for async pipelines. | Absent |
| 1.1.3 | `cudaStreamAddCallback` | High — legacy but still used by some frameworks. | Absent |
...
1. **Immediate (blocks PyTorch/TF graphs)**: Implement `cudaGraphAddKernelNode`, `cudaStreamWaitEvent`, `cudaMemcpyToSymbol`, `cudaEventQuery`, `cudaFuncGetAttributes`.
