...
| cuDNN | ~26% | 40 functions implemented; 120+ missing. Forward: conv, activation, pooling, softmax, BN inference. Backward: activation, conv backward-data/backward-filter. Missing: BN training/backward, dropout, RNN/LSTM/GRU, attention, CTC loss, `OpTensor`, `ReduceTensor`, `TransformTensor`. |
