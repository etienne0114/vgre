...
- **cuDNN** (~28% coverage, ~42/~150+ functions): Forward: conv, pooling, activation (ReLU, sigmoid, tanh, ELU, GELU, SELU, Mish), softmax, BN inference/training. Backward: activation, conv backward-data/backward-filter, BN backward. **Missing**: dropout, RNN/LSTM/GRU, attention, `OpTensor`, `ReduceTensor`.
...
- **cuDNN**: ~28% coverage. Forward complete (including BN training). Backward: activation, conv backward-data/backward-filter, BN backward. Missing: dropout, RNN/LSTM/GRU, attention, `OpTensor`, `ReduceTensor`.
...
- **cuDNN**: dropout, RNN/LSTM/GRU, attention, `OpTensor`, `ReduceTensor`. Activation backward, conv backward-data/backward-filter, and BN forward/backward now complete.
