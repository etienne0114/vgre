...
- **cuDNN**: ~37% coverage. Forward complete (including BN training, dropout). Backward: activation, conv backward-data/backward-filter, BN backward, softmax backward, pooling backward, dropout. Utility: OpTensor, ReduceTensor, TransformTensor. Missing: RNN/LSTM/GRU, attention, CTC loss.
...
- **cuDNN**: RNN/LSTM/GRU, attention, CTC loss. All core forward/backward passes and utility ops (OpTensor, ReduceTensor, TransformTensor) now complete.
