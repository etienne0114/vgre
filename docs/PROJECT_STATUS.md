...
- **cuDNN** (~27% coverage, ~41/~150+ functions): Forward: conv, pooling, activation (ReLU, sigmoid, tanh, ELU, GELU, SELU, Mish), softmax, BN inference/training. Backward: activation, conv backward-data/backward-filter. **Missing**: BN backward, dropout, RNN/LSTM/GRU, attention, `OpTensor`, `ReduceTensor`.
...
- **cuDNN**: ~27% coverage. Forward complete (including BN training). Backward: activation, conv backward-data/backward-filter. Missing: BN backward, dropout, RNN/LSTM/GRU, attention, `OpTensor`, `ReduceTensor`.
...
- **cuDNN**: BN backward, dropout, RNN/LSTM/GRU, attention, `OpTensor`, `ReduceTensor`. Activation backward, conv backward-data/backward-filter, and BN forward training now complete.
