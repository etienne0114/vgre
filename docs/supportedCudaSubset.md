...
| **memory fences** — `__threadfence_block()`, `__threadfence()`, `__threadfence_system()` | ✅ Lowered to PTX `membar.cta`/`membar.gl`/`membar.sys`. The cooperative Tier-0 interpreter runs memory ops in program order over one shared address space, so every prior write is already visible — the fence is a semantic no-op there — but producer/consumer kernels that name it compile and run unchanged. |
