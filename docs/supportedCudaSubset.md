...
| **warp reduce** — `__reduce_{add,min,max,and,or,xor}_sync` | ✅ Tier-0 interpreter. Lowers to `redux.sync.<op>.<type>` (add/min/max honour the value's signedness → `s32`/`u32`; bitwise → `b32`); the warp rendezvous folds `value` over the lanes selected by the membership mask and delivers the result to every calling lane. Warp-cooperative → Tier-0. |
