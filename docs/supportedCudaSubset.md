...
> hand-emitted machine code, the default in the no-LLVM build) for the scalar/
> elementwise/GEMM subset, else the **Tier-1 compiled** backend (`compiled_kernel.cpp`)
> — bound closures, plus a **fiber executor** that runs the whole cooperative surface
> (`__shared__`, `__syncthreads`, `__syncwarp`, `__shfl_*`, warp vote/reduce/match,
> `__syncthreads_{count,and,or}`, `__activemask`) — else the **Tier-0 PTX interpreter**
> (`ptx_interpreter.cpp`, the guaranteed fallback). The native tier covers the scalar subset — every int/float
...
checked by **differential fuzzers**: thousands of random valid kernels are
compiled, run on the interpreter, and compared element-by-element against an
independent reference — so a silently-wrong codegen or fold is a test failure,
not a latent bug. `tests/compiler/test_cuda_fuzz_diff.cpp` covers signed-integer
expressions; `test_cuda_fuzz_float.cpp` and `test_cuda_fuzz_double.cpp` cover
`float` and `double` arithmetic (`+ - * /`, unary `-`), which is **bit-exact**
against the host — the front-end lowers to `add.f32`/`mul.f32`/`div.rn.f32` (and
the `.f64` forms) with no fma contraction and no `.ftz`, so every op is a single
IEEE-754 rounding step. `test_cuda_fuzz_cast.cpp` covers **mixed int/float/double**
expressions with widening casts — the `cvt` conversion class and
usual-arithmetic-conversion promotion — also bit-exact (every conversion used is
exact or single-rounded). `test_cuda_fuzz_toint.cpp` covers **float/double → int**,
...
interpreter implements exactly that. Finally eight cross-tier fuzzers run each random
...
`test_cuda_fuzz_structarr.cpp` over **struct arrays** (`arr[i].field` reads +
writes), and `test_cuda_fuzz_ptr.cpp` over **explicit pointer arithmetic**
(`*(a+j)`, `*(j+a)`, `p-q`, `*p = v` — element-scaled). The PTX header's
...
| **built-in vector types** — `float{1,2,3,4}`, `int/uint/char/uchar/short/ushort/long/ulong{1,2,3,4}`, `double{1,2,3,4}` | ✅ **on both tiers**. Modelled as auto-registered structs with `x`/`y`/`z`/`w` components (registered on first use, natural CUDA layout), so all the struct machinery applies: `make_<T>(…)` constructors, `.x/.y/.z/.w` access, and **vectorized whole-struct loads/stores** — `float4 v = p[i];` and `p[i] = v;` (or `= make_float4(…)`) move all lanes at once. Held bit-exact compiled-tier == interpreter == reference by `test_cuda_vectypes.cpp`. (No built-in vector **operators** — CUDA itself needs `helper_math.h` for `float4 + float4`; write component-wise.) |
| **texture / surface fetches** — `tex1D`, `tex2D`, `tex3D`, `tex1Dfetch`, `surf2Dread`, `surf2Dwrite` (incl. the `tex2D<float>` object-API spelling) | ✅ **on both tiers**, scalar-`float` path. Handles are `cudaTextureObject_t`/`cudaSurfaceObject_t` (opaque `u64` params). Both tiers dispatch to the shared `TextureManager` (address/filter modes, normalized coords, mipmaps) that the LLVM tier already uses: the **Tier-1 compiled** backend calls it from a bound closure; the **Tier-0 interpreter** lowers to in-house `vgretex*`/`vgresurf*` PTX ops that call the same sampler — so both match `TextureManager` bit-for-bit (`test_cuda_texture.cpp`). |
...
| `__shared__` arrays + `__syncthreads()` | ✅ On the **Tier-1 compiled fiber executor** (fast) as well as the Tier-0 interpreter: each CUDA thread runs as a stackful fiber, and a per-block scheduler releases `__syncthreads()` once every thread arrives. Barriers anywhere (loops, ifs) work with no kernel transformation; `__shared__` is a block-wide buffer. Held bit-exact against the interpreter by `test_cuda_coop.cpp` (block reduction, tiled GEMM). ~30–200× faster than the interpreter. Portable to every host (`ucontext` on POSIX incl. macOS, the Win32 Fibers API on Windows) — no interpreter fallback. Dynamic/scalar `__shared__` still uses Tier-0. |
...
| **warp shuffle** — `__shfl_sync`, `__shfl_up_sync`, `__shfl_down_sync`, `__shfl_xor_sync` | ✅ Tier-0 interpreter (32-lane warp rendezvous; optional power-of-two `width`). **32-bit and 64-bit** values: `double`/`long` shuffle as two `.b32` words (low/high) recombined — bit-exact (a `double` moves through an integer register, shuffling its raw IEEE-754 bits). Runs on the **compiled fiber tier** (per-warp rendezvous with a publish→snapshot buffer, matching the interpreter's width/subgroup/wrap rules) as well as Tier-0 — bit-exact, see `test_cuda_coop.cpp`. |
| **warp vote** — `__ballot_sync`, `__any_sync`, `__all_sync` | ✅ Tier-0 interpreter. The predicate becomes a real `.pred` (`setp.ne`); a warp-wide rendezvous collects every active lane's bit into a ballot mask, then each lane reads its result (`ballot` = the masked lane bitmask; `any`/`all` = reductions over the membership mask). Runs on the **compiled fiber tier** and Tier-0, bit-exact. |
| **warp reduce** — `__reduce_{add,min,max,and,or,xor}_sync` | ✅ Tier-0 interpreter. Lowers to `redux.sync.<op>.<type>` (add/min/max honour the value's signedness → `s32`/`u32`; bitwise → `b32`); the warp rendezvous folds `value` over the lanes selected by the membership mask and delivers the result to every calling lane. Runs on the **compiled fiber tier** and Tier-0, bit-exact. |
| **warp match** — `__match_any_sync(mask, v)`, `__match_all_sync(mask, v, &pred)` (sm_70+) | ✅ Tier-0 interpreter. Lowers to `match.any.sync.b32`/`match.all.sync.b32` (32-bit key); the warp rendezvous snapshots every lane's key. `any` → each lane gets the bitmask of membership-mask lanes holding **its** key; `all` → the membership mask if every participating lane's key agrees (else 0), with `*pred` set to that all-agree flag. Runs on the **compiled fiber tier** (publish→snapshot per-warp compare; `&pred` as a local int or `&out[i]`) and Tier-0, bit-exact — see `test_cuda_coop.cpp`. |
| **warp sync** — `__syncwarp([mask])`, `__activemask()` | ✅ Tier-0 interpreter. `__syncwarp` → `bar.warp.sync`, a 32-lane-scoped barrier (all active lanes rendezvous before continuing); `__activemask` → `activemask.b32`, the bitmask of the warp's non-exited lanes. Runs on the **compiled fiber tier** and Tier-0, bit-exact — the fiber scheduler resumes lanes in order, reproducing the interpreter's sequential live-lane mask. |
| **block barrier reductions** — `__syncthreads_count(pred)`, `__syncthreads_and(pred)`, `__syncthreads_or(pred)` | ✅ Tier-0 interpreter. A CTA-wide barrier fused with a reduction over the predicate: `__syncthreads_count` → `bar.red.popc.u32` (number of threads with a nonzero predicate); `__syncthreads_and`/`_or` → `bar.red.and`/`or.pred` (nonzero iff the predicate holds for **all** / **any** thread). The whole block rendezvouses, the parked threads' predicates are reduced, and the result is broadcast to every thread. Runs on the **compiled fiber tier** (the block-barrier release computes count/any/all) and Tier-0, bit-exact. |
...
`__ballot`/`__any`/`__all` — which the parser desugars to their `_sync` forms with an
implicit full-warp mask (0xffffffff), so all three backends run them (the whole `_sync`
surface — `__shfl_*_sync`, `__ballot_sync`/`__any_sync`/`__all_sync`, `__reduce_*_sync`,
`__match_*_sync`, `__activemask`, incl. 64-bit `__shfl_*_sync` of `double`/`long` — was
already supported); **recursion** in `__device__` helpers incl. **mutual recursion** via
function prototypes/forward declarations (Tier-1 compiled); the full **texture/surface**
surface — vector fetches `texND<float4>`, `tex2DLod`, layered/cubemap/cubemap-array/
layered-LOD sampling; and PTX `mul.hi.64`.)*
