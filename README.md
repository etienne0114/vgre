...
> **PROJECT STATUS** (2026-09): **All three platforms green in CI — Linux production-verified, macOS ARM64 CI-green, Windows x86-64 CI-green.** The full `ctest` suite (325 tests) passes on **x86-64 Linux** (the required CI job); CUDA, BLAS, DNN, FFT/RNG/solver/sparse, NCCL, profiling, and distributed-cluster paths are implemented with real CPU math (no runtime stubs). CI runs free on the public repo on **every push** — the macOS (Apple Silicon, auto-detected Homebrew `llvm@18`) and Windows (clang-cl on `windows-2022`) jobs both build **and run the whole `ctest` suite** to green (confirmed 2026-09-18); they remain informational/`continue-on-error` in the workflow pending a few more consecutive green runs before promotion to required. A **free public demo** runs on CPU at **https://vgrengine.streamlit.app**. Hardware-only boundaries (physical GPU PMU counters, Metal MPS backend, GPUDirect RDMA) are in `docs/missingFeatures.md`. A plan to make the heavy **LLVM dependency optional** (lightweight, Windows-friendly, interpreter-based builds) is in [`docs/vNext_audit_and_llvm_reduction.md`](docs/vNext_audit_and_llvm_reduction.md).
