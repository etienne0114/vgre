...
> **PROJECT STATUS** (2026-09): **All three platforms green in CI — Linux production-verified, macOS ARM64 CI-green, Windows x86-64 CI-green.** The full `ctest` suite passes on **x86-64 Linux**: **394 tests with LLVM, 374 in the LLVM-free build** (`VGRE_ENABLE_JIT=OFF`, `VGRE_ENABLE_OPENMP=OFF`) — the "runs on any machine, no toolchain" path is guarded on every push, not just built by hand. CUDA, BLAS, DNN, FFT/RNG/solver/sparse, NCCL, profiling, and distributed-cluster paths are implemented with real CPU math (**no runtime stubs**). **VGRE no longer needs LLVM to execute kernels:** a **from-scratch CUDA-C front-end** (own lexer/parser → AST) feeds a four-tier CPU execution stack — a **PTX interpreter** (Tier 0), a **compiled-closure backend with a cooperative fiber executor** (Tier 1, `__shared__`/`__syncthreads`/warp intrinsics), a **native x86-64 machine-code JIT** (Tier 1b, ~18–118× the compiled tier), and an **optional own SSA optimizing backend** (Tier 2 / `VGRE_EXEC_BACKEND=ssa`: VGRE-IR → const-fold/GVN/LICM/DCE → linear-scan regalloc → x86-64 native + opt-in AArch64, feature-complete for the scalar + shared-memory + warp subset). Every tier is held **bit-exact** against the others. CI runs free on the public repo on **every push**; the macOS (Apple Silicon, auto-detected Homebrew `llvm@18`) and Windows (clang-cl on `windows-2022`) jobs both build **and run the whole `ctest` suite** to green, remaining `continue-on-error` pending promotion. A **free public demo** runs on CPU at **https://vgrengine.streamlit.app**. Hardware-only boundaries (physical GPU PMU counters, Metal MPS backend, GPUDirect RDMA) are in `docs/missingFeatures.md`.

## What is VGRE?

VGRE intercepts CUDA and OpenCL API calls and executes kernels on CPU using:
- A **from-scratch, LLVM-free execution stack** (the default in the zero-burden build): own CUDA-C lexer/parser → AST → **PTX interpreter** / **compiled-closure + fiber tier** / **native x86-64 JIT** / **own SSA optimizing backend** (`VGRE_EXEC_BACKEND=interp|cp|ssa`), each held bit-exact against the others — no toolchain required to run kernels
- LLVM-18 ORC JIT (Clang AST parsing → LLVM IR → native code, `-O3 -march=native -fno-math-errno`) — the optional high-performance path when LLVM dev libs are present
...
### Recent Improvements (2026-09) 🎉 — the from-scratch, LLVM-free execution stack
- ✅ **Own CUDA-C front-end** — a hand-written lexer/parser (no Clang) lowers CUDA-C kernels to an AST, then to PTX or directly to the compiled/native tiers. Covers the practical compute subset with **located errors** (source line + caret) for anything outside it.
...
- ✅ **LLVM made optional** — kernel execution no longer requires LLVM; the `VGRE_ENABLE_JIT=OFF` build runs the full 374-test suite green using only the from-scratch stack.
...
- [`docs/zeroBurdenRoadmap.md`](docs/zeroBurdenRoadmap.md) — The from-scratch, LLVM-free execution stack (own CUDA-C front-end + four CPU tiers incl. the Tier-2 SSA backend) and what remains
- [`docs/supportedCudaSubset.md`](docs/supportedCudaSubset.md) — The exact CUDA-C subset the from-scratch tiers accept, per feature, with the tier(s) that run each
